...
@implementation SDInputToolbarConfigure

+ (instancetype)defaultConfigure {
    SDInputToolbarConfigure *configure = [[SDInputToolbarConfigure alloc] init];
...
@interface SDInputToolbar ()<UITextViewDelegate>

@property (nonatomic, strong) SDInputToolbarConfigure *configure;
/**遮罩*/
...
- (void)updateWithConfig:(void(^)(SDInputToolbarConfigure *configure))configBlock {
    if (configBlock) {
...
-(void)layoutSubviews {
    ...
    self.maskView.cl_size = CGSizeMake(Screen_Width, Screen_Height);
    self.cl_width = Screen_Width;
...
//MARK:JmoVxia---懒加载
- (SDInputToolbarConfigure *) configure {
    if (_configure == nil) {
        _configure = [SDInputToolbarConfigure defaultConfigure];
    }
    return _configure;
}
- (UIView *) maskView {
